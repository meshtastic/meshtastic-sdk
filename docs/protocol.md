...
1. The phone MUST consider itself **`Configuring`** until `config_complete_id == 69421` (or matching sentinel for the legacy single-stage flow). It MAY surface configs/channels/nodes as they stream in, but MUST NOT consider itself `Connected` until Stage 2 ends.
   - **Config-only exception.** A host that deliberately never requests the NodeDB may stop after the Stage 1 sentinel: the firmware enters `STATE_SEND_PACKETS` once it emits `config_complete_id = 69420`, so normal traffic flows without Stage 2. Such a session knows only the device's own `NodeInfo` (row 3) until peers are heard live. The SDK exposes this as `RadioClient.Builder.skipNodeDb()` / `configOnly()`.
