...
#### Config-only connect — `skipNodeDb()` / `configOnly()` *(since 0.3.0)*
...
`skipNodeDb()` (and its intent-named alias `configOnly()`) stops after Stage 1: `69421` is never
sent, and the session goes straight to seeding the admin session passkey and then `Connected`.
