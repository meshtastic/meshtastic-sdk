...
class P2TelemetryRpcTest {
    ...
    fun requestDeviceReturnsMatchingArm() = runTest {
        ...
        val expected = DeviceMetrics.Builder().also { wb ->wb.battery_level = 87; wb.voltage = 4.05f; wb.uptime_seconds = 3600}.build()
    ...
    fun requestEnvironmentMissingArmReportsFailed() = runTest {
        ...
        transport.injectTelemetryResponse(
            requestId = req.id,
            telemetry = Telemetry.Builder().also { wb ->wb.device_metrics = DeviceMetrics.Builder().also { wb ->wb.battery_level = 50}.build()}.build(),
        )
        runCurrent()
    ...
    fun observeFiltersByNodeAndPortnum() = runTest {
        ...
        transport.injectTelemetryResponse(
            requestId = 0,
            telemetry = Telemetry.Builder().also { wb ->wb.local_stats = LocalStats.Builder().also { wb ->wb.uptime_seconds = 1}.build()}.build(),
            fromNode = 0x42,
        )
    ...
    fun observeLocalAcceptsAnyOrigin() = runTest {
        ...
        runCurrent()
        val telem = Telemetry.Builder().also { wb ->wb.device_metrics = DeviceMetrics.Builder().also { wb ->wb.battery_level = 12}.build()}.build()
