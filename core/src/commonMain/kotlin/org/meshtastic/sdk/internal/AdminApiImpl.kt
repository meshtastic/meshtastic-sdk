...
internal class AdminApiImpl(
    ...
    override suspend fun getDeviceMetadata(): AdminResult<DeviceMetadata> = retryOnSessionExpiry {
        submitAdminRpc(
            adminMsg = AdminMessage.Builder().also { wb ->wb.get_device_metadata_request = true}.build(),
    ...
    override suspend fun getModuleConfig(type: AdminMessage.ModuleConfigType): AdminResult<ModuleConfig> =
        retryOnSessionExpiry {
            submitAdminRpc(
                adminMsg = AdminMessage.Builder().also { wb ->wb.get_module_config_request = type}.build(),
    ...
    override suspend fun getChannel(index: ChannelIndex): AdminResult<Channel> = retryOnSessionExpiry {
        submitAdminRpc(
            ...
            adminMsg = AdminMessage.Builder().also { wb ->wb.get_channel_request = index.raw + 1}.build(),
            kind = ResponseKind.AdminChannel,
        )
    }

    override suspend fun setChannel(channel: Channel): AdminResult<Unit> {
        val result = retryOnSessionExpiry { submitAdminAck(AdminMessage.Builder().also { wb ->wb.set_channel = channel}.build()) }
    ...
    override suspend fun getCannedMessages(): AdminResult<String> = retryOnSessionExpiry {
        submitAdminRpc(
            adminMsg = AdminMessage.Builder().also { wb ->wb.get_canned_message_module_messages_request = true}.build(),
    ...
    override suspend fun setCannedMessages(messages: String): AdminResult<Unit> = retryOnSessionExpiry {
        submitAdminAck(AdminMessage.Builder().also { wb ->wb.set_canned_message_module_messages = messages}.build())
    }
    ...
    override suspend fun getDeviceConnectionStatus(): AdminResult<DeviceConnectionStatus> = retryOnSessionExpiry {
        submitAdminRpc(
            adminMsg = AdminMessage.Builder().also { wb ->wb.get_device_connection_status_request = true}.build(),
    ...
    override suspend fun getRemoteHardwarePins(): AdminResult<NodeRemoteHardwarePinsResponse> = retryOnSessionExpiry {
        submitAdminRpc(
            adminMsg = AdminMessage.Builder().also { wb ->wb.get_node_remote_hardware_pins_request = true}.build(),
    ...
    override suspend fun enterDfuMode(): AdminResult<Unit> {
        ...
        return submitAdminFireAndForget(AdminMessage.Builder().also { wb ->wb.enter_dfu_mode_request = true}.build())
    }
    ...
    override suspend fun backupPreferences(location: AdminMessage.BackupLocation): AdminResult<Unit> =
        retryOnSessionExpiry {
            submitAdminAck(AdminMessage.Builder().also { wb ->wb.backup_preferences = location}.build())
        }

    override suspend fun restorePreferences(location: AdminMessage.BackupLocation): AdminResult<Unit> =
        retryOnSessionExpiry {
            submitAdminAck(AdminMessage.Builder().also { wb ->wb.restore_preferences = location}.build())
        }

    override suspend fun removeBackupPreferences(location: AdminMessage.BackupLocation): AdminResult<Unit> =
        retryOnSessionExpiry {
            submitAdminAck(AdminMessage.Builder().also { wb ->wb.remove_backup_preferences = location}.build())
        }
    ...
    override suspend fun keyVerification(verification: KeyVerificationAdmin): AdminResult<Unit> = retryOnSessionExpiry {
        submitAdminAck(AdminMessage.Builder().also { wb ->wb.key_verification = verification}.build())
    }
    ...
    override suspend fun lockdown(auth: LockdownAuth): AdminResult<Unit> {
        ...
        return submitAdminFireAndForget(AdminMessage.Builder().also { wb ->wb.lockdown_auth = auth}.build(), to = NodeId(local))
    }

    // ── OTA updates ─────────────────────────────────────────────────────────

    override suspend fun rebootOta(after: Duration): AdminResult<Unit> = retryOnSessionExpiry {
        submitAdminAck(AdminMessage.Builder().also { wb ->wb.reboot_ota_seconds = after.inWholeSeconds.toInt().coerceAtLeast(0)}.build())
    }
    ...
    override suspend fun reboot(after: Duration): AdminResult<Unit> = retryOnSessionExpiry {
        submitAdminAck(AdminMessage.Builder().also { wb ->wb.reboot_seconds = after.inWholeSeconds.toInt().coerceAtLeast(0)}.build())
    }

    override suspend fun shutdown(after: Duration): AdminResult<Unit> = retryOnSessionExpiry {
        submitAdminAck(AdminMessage.Builder().also { wb ->wb.shutdown_seconds = after.inWholeSeconds.toInt().coerceAtLeast(0)}.build())
    }
    ...
    override suspend fun setTimeOnly(unixTime: Int): AdminResult<Unit> {
        if (isLocalTargetManaged()) return AdminResult.Unauthorized
        return submitAdminFireAndForget(AdminMessage.Builder().also { wb ->wb.set_time_only = unixTime}.build())
    }
    ...
    private suspend fun beginEditSettings(): AdminResult<Unit> =
        retryOnSessionExpiry { submitAdminAck(AdminMessage.Builder().also { wb ->wb.begin_edit_settings = true}.build()) }

    private suspend fun commitEditSettings(): AdminResult<Unit> =
        retryOnSessionExpiry { submitAdminAck(AdminMessage.Builder().also { wb ->wb.commit_edit_settings = true}.build()) }
    ...
    private inner class AdminEditImpl : AdminEdit {
        ...
        override suspend fun setModuleConfig(config: ModuleConfig) {
            enqueueOrThrow(AdminMessage.Builder().also { wb ->wb.set_module_config = config}.build())
            writtenModuleConfigs += config
        }
        override suspend fun setOwner(user: User) = enqueueOrThrow(AdminMessage.Builder().also { wb ->wb.set_owner = user}.build())
        override suspend fun setChannel(channel: Channel) = enqueueOrThrow(AdminMessage.Builder().also { wb ->wb.set_channel = channel}.build())
